...
async def start(msg: types.Message):
    ...
    if args:
        ...
        files = [i[0] for i in cur.fetchall()]

        await send_files(msg.chat.id,files,0)
...
async def verify(call: types.CallbackQuery):
    ...
    if await check_join(call.from_user.id):
        ...
        await call.message.edit_text("Verified. Bot bisa digunakan")

    else:

        await call.answer("Belum join group",show_alert=True)
...
async def save(msg: types.Message):
    ...
    user_files[uid].append(sent.message_id)

    await progress.edit_text(
        f"Uploaded {len(user_files[uid])} file"
    )
